...
##CONFIGURAÇÕES
####  TITULO DO MENU  #################################################################
title_menu = '[B][COLOR white]:::[/COLOR] [COLOR aqua] BEM-VINDOS [/COLOR] [COLOR white]:::[/COLOR][/B]'
###  DESCRIÇÃO DO ADDON ###############################################################
title_descricao = '[B][COLOR beige]CONE[B][COLOR aqua][B]X[/B][COLOR  beige][B]ÃO[/COLOR][COLOR aqua][B] PLAY[/COLOR][/B] - [COLOR beige][B]Melhor conteudo de filmes e séries para você e toda sua família[/COLOR][/B]'

####  LINK DO TITULO DE MENU  #########################################################
## OBS: POR PADRÃO JÁ TEM UM MENU EM BRANCO PARA NÃO TER ERRO AO CLICAR ###############
url_b64_title = 'https://cld.pt/dl/download/eec25d8e-da69-4ce0-b400-d4d14de4ad96/homemultcon.xml?download=true'
#url_title = base64.b64decode(url_b64_title).decode('utf-8')
url_title = ''


##### PESQUISA - get.php
#url_b64_pesquisa = 'https://github.com/Anubis-team/An-bis/find/master'
#url_pesquisa = base64.b64decode(url_b64_pesquisa).decode('utf-8')
url_pesquisa = 'https://github.com/Anubis-team/An-bis/find/master'
...
## MENU CONFIGURAÇÕES
menu_configuracoes = '[B][COLOR red]»» [COLOR aqua]CONFIGURAÇÕES[/COLOR][/B]'
thumb_icon_config = 'https://images.vexels.com/media/users/3/145590/isolated/preview/a43a2b2c7d41d6d0e91c7777aa37d1e1-roda-dentada-by-vexels.png'
desc_configuracoes = 'Configurações do conexao'
## FAVORITOS
menu_favoritos = '[COLOR  yellow][B]»» [COLOR beige]FAVORITOS[/COLOR][/B]'
thumb_favoritos = 'https://www.icone-png.com/png/9/8584.png'
desc_favoritos = '[COLOR  yellow]Adicione Itens aos Favoritos, pressionando OK do controle[/COLOR]'

#### POP UP SUPORTE
titulo_suporte = '[COLOR aqua][B]BEM VINDOS AO [B][COLOR beige]CON[B][COLOR aqua][B]X[/B][COLOR  beige][B]ÃO[/COLOR][COLOR aqua][B] PLAY[/B][/COLOR]'

### MENSAGEM STATUS
mensagem_status = '[COLOR lime][B] ● [COLOR red][B]TV   [COLOR lime][B] ● [COLOR gold][B]FILMES  [COLOR lime][B] ● [COLOR blue][B]SÉRIES [COLOR lime][B] ● [COLOR magenta][B]DESENHOS [COLOR lime][B] ● [COLOR aqua][B]NOVELAS [COLOR yellow]E [COLOR red]MAIS!!![/COLOR][/B]'

#### MENU VIP ################################################################
titulo_vip = '[COLOR  yellow][B]»» [COLOR aquamarine][B] CANAIS[/B][/COLOR] [COLOR yellow][B](VIPS)[/B][/COLOR]'
thumbnail_vip = 'https://i.postimg.cc/DyQzG5MY/vips.png'
fanart_vip = 'https://i.postimg.cc/d1F3tmgG/Whats-App-Image-2021-04-30-at-13-43-34.jpg'
#### DESCRIÇÃO VIP ###########################################################
vip_descricao = '[COLOR aqua][B]SOLICITE SEU ACESSO VIP, O MENOR PREÇO DE TODOS -APARTIR DE [COLOR red][B]15,00 REAIS[/B][/COLOR]'
#### DIALOGO VIP - SERVIDOR ATIVADO - CLICK ###################################
vip_dialogo = '[B][COLOR aqua]CONFIGURE SEU ACESSO NA NOSSA ÁREA VIP ,ENTRE EM CONTATO PELO WHATSAPP: [COLOR red][B]55+ (82) 98131-4468[COLOR aqua][B] E SOLICITE SEU ACESSO[/B][/COLOR]'
##SERIVODR VIP
url_server_vip = 'http://actxs.biz:80'


## MULTLINK
## nome para $nome, padrão: nome para $nome
playlist_command = 'nome'
dialog_playlist = '[COLOR lime][B]SELECIONE [COLOR aqua] [B]UMA [COLOR aqua]OPÇÃO[/B][/COLOR]'
...
url_b64_principal = 'aHR0cHM6Ly9jbGQucHQvZGwvZG93bmxvYWQvOGQ2YjYyYjEtZjZlMi00YTQwLWIxZmMtMDhiMjUyYWYxYmQ2L25vdm8lMjBjYXRlZ29yaXMueG1sP2Rvd25sb2FkPXRydWU='
...
def getData(url,fanart,pesquisa=False):
    ...
    if isinstance(data, (int, str, list)):
        channels = re.compile('<channels>(.+?)',re.MULTILINE|re.DOTALL).findall(data)
...
def get_m3u8(url,data):
    content = data.rstrip()
    if url.startswith(url_server_vip) == True:
        addDir('[COLOR aqua][B]Status da Conta[/B][/COLOR] ','',22,thumbnail_vip,fanart_vip,'Informação da conta','','','','','','','','','','','','','','','','')
    try:
    ...
    for item in items:
        ...
        try:
            ...
            ##Multilink
                ...
                name_resolve = name+'[COLOR red] ('+str(len(url2))+'[/COLOR]'
                ...
                name_resolve = name+'[COLOR red] '+' [/COLOR]'
...
def getFavorites():
    try:
        ...
        if int(total) > 0:
            ...
            xbmcgui.Dialog().ok('[COLOR aqua]AVISO[/COLOR][/B]','[COLOR yellow][B]Nada Adicionado nos Favoritos','Nada Adicionado nos Favoritos')
    except:
        pass
...
def addFavorite(name,url,fav_mode,subtitle,iconimage,fanart,description):
    ...
    if os.path.exists(favorites)==False:
        ...
        b.close()
        notify('[COLOR lime]Adicionado aos Favoritos do[/COLOR] '+__addonname__)
        #xbmc.executebuiltin("XBMC.Container.Refresh")
...
def rmFavorite(name):
    ...
    for index in range(len(data)):
        if data[index][0]==name:
            ...
            b.close()
            break
    notify('[COLOR red][B]Removido dos Favoritos do[/B][/COLOR] '+__addonname__)
    #xbmc.executebuiltin("XBMC.Container.Refresh")
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,year,director,writer,duration,premiered,studio,rate,originaltitle,country,rating,userrating,votes,aired,folder=True):
    ...
    try:
        ...
        if favoritos == 'true' and  mode !=4 and mode !=7 and mode !=8 and mode !=9 and mode !=10 and mode !=12 and mode !=15:
            if name_fav in FAV:
                contextMenu.append(('[COLOR yellow][B]Remover dos Favoritos do[/B][/COLOR] '+__addonname__,'RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name))))
            else:
                ...
                contextMenu.append(('[COLOR lime][B]Adicionar aos Favoritos do[/B][/COLOR] '+__addonname__,'RunPlugin(%s)' %fav_params))
...
def check_addon():
    try:
        ...
        if exists == True:
                    ...
                    if addon_id == 'plugin.video.elementum' and elementum == 'false':
                        ...
                        if os.path.exists(existe)==False:
                            ...
                            if addon_id == 'plugin.video.elementum':
                                xbmcgui.Dialog().ok('[B][COLOR yellow]AVISO[/COLOR][/B]','[COLOR aqua]FECHE O KODI E ABRA NOVAMENTE PARA ATIVAR O ELEMENTUM[/COLOR]')
                        else:
                            pass
            ...
            fcheck.write('1')
            fcheck.close()
            xbmcgui.Dialog().ok('[B][COLOR yellow]AVISO[/COLOR][/B]','[COLOR aqua]FECHE O KODI E ABRA NOVAMENTE PARA VERIFICAR COMPLEMENTOS[/COLOR]')
    except:
        pass
...
def status():
    ...
    if max_connections > '':
        ...
        limite_conexao = ''
    msg = 'Status: [COLOR aquamarine]%s[/COLOR]\nExpira em: [COLOR aquamarine]%s[/COLOR]\nDemonstrativo: [COLOR aquamarine]%s[/COLOR]\nCriado em: [COLOR aquamarine]%s[/COLOR]\nConexões permitidas: [COLOR aquamarine]%s[/COLOR]'%(status_result,str(expires),vip_trial,str(created_time),str(limite_conexao))
    xbmcgui.Dialog().textviewer('[COLOR aqua][B] Conta Premium:[/B][/COLOR]', msg)         
...
def suporte():
    ...
    import webbrowser
    dialog = xbmcgui.Dialog()
    link_vip = 'https://api.whatsapp.com/send/?phone=5582981314468&text=Solicito+login+teste+de+acesso+VIP+Conex%C3%A3o+Play&app_absent=0'
    link_facebook = 'https://www.facebook.com/groups/511580836167016/?epa=SEARCH_BOX'
    link_telegram = 'https://t.me/joinchat/l3yA2MGOetExZDdh'
    link = dialog.select(titulo_suporte, ['[B][COLOR beige]CONE[B][COLOR aqua][B]X[/B][COLOR  beige][B]ÃO[/COLOR][COLOR aqua][B] PLAY[/B][/COLOR] [COLOR yellow][B] VIP[/B][/COLOR]','[B][B][COLOR beige]CONE[B][COLOR aqua][B]X[/B][COLOR  beige][B]ÃO[/COLOR][COLOR aqua][B] PLAY[/B][/COLOR] [COLOR blue][B] FACEBOOK[/B][/COLOR]','[B][COLOR beige]CONE[B][COLOR aqua][B]X[/B][COLOR  beige][B]ÃO[/COLOR][COLOR aqua][B] PLAY[/B][/COLOR] [COLOR lightblue][B] TELEGRAM [/COLOR][/B]','[COLOR aqua][B]ENTRAR NO ADDON AQUI[/COLOR][/B]'])
    if link == 0:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %(link_vip))
        else:
            webbrowser.open(link_vip)
    if link == 1:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %(link_facebook))
        else:
            webbrowser.open(link_facebook)
    if link == 2:
        if xbmc.getCondVisibility('system.platform.android'):
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' %(link_telegram))
        else:
            webbrowser.open(link_telegram)            
...
def SKindex():
    ...
    if addon.getSetting('exibirvip') == 'true':
        addDir(titulo_vip,'',20,thumbnail_vip,fanart_vip,vip_descricao,'','','','','','','','','','','','','','','','')
    if favoritos == 'true':
...
#Configurações
    ...
    xbmcaddon.Addon().openSettings()
    xbmcgui.Dialog().ok('[B][COLOR yellow]AVISO[/COLOR][/B]','[COLOR aqua]FECHE O KODI E ABRA NOVAMENTE PARA ATUALIZAR AS CONFIGURAÇÕES[/COLOR]')
    xbmc.executebuiltin("XBMC.Container.Refresh()")
